...
--content-max: 480px;   /* max on 기본값 */

.card { width: max(var(--content-min), var(--content-raw)); }
body[data-width="fluid"] .card { max-width: var(--content-max); }
```

원래는 12컬럼 그리드 기반 `(2.5 × 컬럼) + (3 × 거터)` (거터 16 / 마진 20) 였습니다.
전개하면 `20.8333vw + 3px` 이라 **21vw 와 사실상 같은 값**이어서 vw 로 단순화했습니다.

| viewport | 그리드 공식 | 21vw | 차이 |
|---|---|---|---|
| 1280 | 269.67px | 268.80px | −0.87px |
| 1440 | 303.00px | 302.40px | −0.60px |
| 1512 | 318.00px | 317.52px | −0.48px |
| 1800 | 378.00px | 378.00px | 0 |
| 1920 | 403.00px | 403.20px | +0.20px |
| 2560 | 536.33px | 537.60px | +1.27px |

1280~2560 전 구간에서 최대 1.3px (0.5% 미만) 차이입니다.
...
| viewport | 21vw | 적용 (min 360 / max 480) |
|---|---|---|
| 768 | 161.28 | **360** (min) |
| 1280 | 268.80 | **360** (min) |
| 1440 | 302.40 | **360** (min) |
| 1512 | 317.52 | **360** (min) |
| 1714 | 359.94 | **360** (min 분기점) |
| 1920 | 403.20 | **403.2** |
| 2286 | 480.06 | **480** (max 분기점) |
| 2560 | 537.60 | **480** (max) |

값이 실제로 움직이는 구간은 **1714 ~ 2286px** 입니다.
...
|---|---|---|
| 768 – 1714 | 360px | min |
| 1920 | 403.2px | 없음 (시안값 403px 과 0.2px 차) |
| 2286 이상 | 480px | max |

실제로 값이 움직이는 구간은 **1714 ~ 2286px** 입니다. 그 아래는 360, 위는 480 고정입니다.
...
| viewport | 가변 (기본: min 360 / max 480) | 가변 (min·max off = 21vw) | 고정 |
|---|---|---|---|
| 768 | 360px (min) | 161.3px | 400px |
| 1280 | 360px (min) | 268.8px | 400px |
| 1440 | 360px (min) | 302.4px | 400px |
| 1920 | 403.2px | 403.2px | 400px |
| 2560 | 480px (max) | 537.6px | 400px |

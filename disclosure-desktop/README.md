...
### 3. 다이얼로그 box — 가로 고정 / 가변 두 가지 안

우하단 데모 패널의 `width — 고정 560 / 가변 vw` 로 즉시 비교할 수 있습니다.
선택은 localStorage 에 저장되며 기본값은 고정입니다.

```css
--dialog-w-fixed: 560px;             /* 고정 안 */
--dialog-w-fluid: 29.1667vw;         /* 가변 안 — 560 / 1920 */
--dialog-w: var(--dialog-w-fixed);   /* 기본 = 고정 */
body[data-dialog-width="fluid"] { --dialog-w: var(--dialog-w-fluid); }
```

`29.1667vw` 는 1920 에서 정확히 560px 이 되는 값입니다 (560 ÷ 1920 = 0.291666…).
...
내부 여백은 두 안이 동일합니다 — 어느 쪽이든 `mid-*` 공식(기준 1920)을 그대로 따릅니다.

세로는 두 안 공통으로 746px 기본에 상한 800px 이고, `max-height: min(800px, 100%)` 를
...
|---|---|---|---|
| dialog (고정) | 560 × 600 (창 높이에 맞춤) | 560 × 746 | 560 × 746 |
| dialog (가변) | 224 × 600 | 420 × 746 | 560 × 746 |
...
- 가변 안에는 min/max 를 걸지 않았습니다. 순수 vw 라 768 에서 224px 까지 좁아지고
